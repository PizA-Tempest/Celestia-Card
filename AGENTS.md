...
- Currently implemented: scaffold, full 78-card deck in `src/data/tarot.json`, and two reading modes — **Daily Card** (choose → flip → interpret) and **Three Card Reading** (Past/Present/Future spread with a combined interpretation) — with upright/reversed meanings, animated cosmos background, and the disclaimer footer.
- Not yet built (all README "Future Features"): 5-card, Lucky Draw modes, Tarot encyclopedia, reading history, favorites, sharing, theming, PWA.
...
- `src/components/` — `TarotCard` (card face presentation), `CardReveal` (3D flip), `CardDeck` (face-down selection grid), `ReadingResult` (interpretation), `ThreeCardSpread` (3-slot Past/Present/Future picker), `ThreeCardResult` (per-position + combined reading).
